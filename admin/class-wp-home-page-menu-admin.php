...
class WP_Home_Page_Menu_Admin {
	...
	function wp_home_page_menu_settings_link($links, $file) {
		...
		if ( false !== strpos( $file, 'wp-home-page-menu' ) ) {
			$mylinks = array(
				'<a href="https://wordpress.org/support/plugin/wp-home-page-menu/">' . __( 'Get Support', 'wp-home-page-menu' ) . '</a>',
	...
	function wp_home_page_menu_setup_notice(){
		...
		if ( $hascaps ) {
			echo '<div class="notice notice-info is-dismissible wp-home-page-menu"><p>' . sprintf( __( 'To configure <em>WP Home Page Menu plugin</em> please visit its <a href="%1$s">configuration page</a> and to get plugin support contact us on <a href="%2$s">plugin support forum</a>.', 'wp-home-page-menu'), 'options-general.php?page=wp_home_page_menu', 'https://wordpress.org/support/plugin/wp-home-page-menu/' ) . '</p></div>';
		}
	}
		...
		<div class="wrap">
			<form id="wp_home_page_menu_options" action="options.php" method="post">
				...
				<div id="after-submit">
					...
					<p>
						<?php esc_html_e( 'Need Help or Have Suggestions?', 'wp-home-page-menu' ); ?> <?php _e( 'contact us on', 'wp-home-page-menu' ); ?> <a href="https://wordpress.org/support/plugin/wp-home-page-menu/" target="_blank"><?php esc_html_e( "Plugin support forum", 'wp-home-page-menu' ); ?>
					</p>
				</div>
			 </form>
		</div>
	...
	function wp_home_page_menu_enable() {
		...
		echo '<input type="checkbox" id="wp_home_page_menu_enable" name="wp_home_page_menu[wp_home_page_menu_enable]" value="1" ' . checked( 1, $options['wp_home_page_menu_enable'], false) . '/>';
		echo '<label for="wp_home_page_menu_enable"> ' . __( 'Enable', 'wp-home-page-menu' ) . '</label>';
	}
	...
	function wp_home_page_menu_locations() {
		...
		if ( ! empty( $menus ) ) {
			...
			foreach ( $menus as $location => $description ) {
				...
				echo '<input type="checkbox" id="wp_home_page_menu_locations' . esc_attr( $location ) . '" name="wp_home_page_menu[wp_home_page_menu_locations]['.esc_attr( $location ).']" value="'.esc_attr( $location ).'" ' . checked( $location, $check_value, false ) . '/>';
				echo '<label for="wp_home_page_menu_locations' . esc_attr( $location ) . '"> ' . esc_html( $description ) . '</label><br />';
			}
		} else {
			_e( 'No navigation menu registered on your site.', 'wp-home-page-menu' );
		}
	}
	...
	function wp_home_page_menu_label() {
		...
		echo '<input type="text" id="wp_home_page_menu_label" name="wp_home_page_menu[wp_home_page_menu_label]" value="' . esc_attr( $options['wp_home_page_menu_label'] ) . '" size="50" />';
	}
	...
	function wp_home_page_menu_title() {
		...
		echo  '<input type="text" id="wp_home_page_menu_title" name="wp_home_page_menu[wp_home_page_menu_title]" value="' . esc_attr( $options['wp_home_page_menu_title'] ) . '" size="50" />';
	}
	...
	function wp_home_page_menu_classes() {
		...
		echo '<input type="text" id="wp_home_page_menu_classes" name="wp_home_page_menu[wp_home_page_menu_classes]" value="' . esc_attr( $options['wp_home_page_menu_classes'] ) . '" size="50" />';
	}
	...
	function wp_home_page_menu_enable_pages_menu() {
		...
		echo '<input type="checkbox" id="wp_home_page_menu_enable_pages_menu" name="wp_home_page_menu[wp_home_page_menu_enable_pages_menu]" value="1" ' . checked( 1, $options['wp_home_page_menu_enable_pages_menu'], false ) . '/>';
		echo '<label for="wp_home_page_menu_enable_pages_menu"> ' . __( 'Yes', 'wp-home-page-menu' ) . '</label>';
	}
